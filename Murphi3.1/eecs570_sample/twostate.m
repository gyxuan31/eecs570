...
const
  ...
  NumVCs: VC1 - VC0 + 1; -- # virtual channel
  NetMax: ProcCount+1;   -- max network size
...
----------------------------------------------------------------------
type
  Proc: scalarset(ProcCount);   -- unordered range of processors  -- processors
  Value: scalarset(ValueCount); -- arbitrary values for tracking coherence 
  Home: enum { HomeType };      -- need enumeration for IsMember calls  -- home node (memory)
  ...
  ProcState:  -- processors cache states
    Record
  ...
  alias p:Procs[n] Do
	...
	ruleset v:Value Do
  	rule "store new value"
    	...
    	==>
 		   p.val := v;      
 		    b := v;  --We use LastWrite to sanity check that reads receive the value of the last write
  	endrule;
	endruleset;
