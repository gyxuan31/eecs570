...
ruleset n:Proc Do
    ...
    rule "First L1 Read"
        (p2.state = L2_only & p.valid = false)
    ==>
        Send(ReadReq,L2Type,n,VC0,UNDEFINED,UNDEFINED,p.EL);
        --p.valid := true;
    /*Send(ReadReq, HomeType, n, VC0, UNDEFINED);*/
    ...
    rule "Additional Sharer Read"
        (p2.state = PRW & p.EL = FALSE & p2.EL = FALSE) 
    ==>
        Send(ReadReq,L2Type,n,VC0,UNDEFINED,UNDEFINED,p.EL);
    ...
    rule "Other L1 Write"
        (p2.state = PR & p.EL = FALSE & p2.EL = FALSE) 
    ==>
        Send(WriteReq,L2Type,n,VC0,v,UNDEFINED,p.EL);
        --p2.val = v;
    ...
    rule "Any Write"
        (p2.state = SR & p.EL = FALSE & p2.EL = FALSE) 
    ==>
        Send(WriteReq,L2Type,n,VC0,v,UNDEFINED,p.EL);
        --p2.val = v;
    ...
    rule "SW Read"
        (p2.state = SW & p.EL = FALSE & p2.EL = TRUE) 
    ==>
        Send(ReadReq,L2Type,n,VC0,UNDEFINED,UNDEFINED,p.EL);
    ...
    rule "SW Evict"
        ((p2.state = SW | p2.state = SR) & p.EL = FALSE) 
    ==>
